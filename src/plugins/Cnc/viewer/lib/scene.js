...
//Export
export const setup = async (canvas) => {
    //Store variables
    state.canvas = canvas;

    //Scene
    state.scene = new Scene();
    state.scene.background = new Color(0xEDF0F4);

    //Light
    state.light = new HemisphereLight(0xffffff, 0x505050);
    state.light.position.set(0, 0, 10);
    state.scene.add(state.light);

    // Plane

    state.plane = new GridHelper(50, 50, 0x979797, 0xC4C4C4);
